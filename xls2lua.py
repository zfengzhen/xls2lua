...
def write_to_lua_script(excel):
	...
	for (sheet_name, sheet) in excel["data"].items():
		...
		for (item_name, item) in sheet.items():
			# 第一个数据会存在
			outfp.write("TypeMap[" + format_str(item[0][0]) + "] = \"" \
				+ format_str(item[0][1]).replace(" ", "_")  + "s\"\n")
			outfp.write("TypeMap[\"" + format_str(item[0][1]).replace(" ", "_") + "s\"] = " \
				+ format_str(item[0][0]) + "\n")
		...
		for (item_name, item) in sheet.items():
			...
			for (row_idx, row) in item.items():
				if row_idx == 0:
					outfp.write(format_str(item_name).replace(" ", "_") + "s = {}\n\n")
				outfp.write(format_str(item_name).replace(" ", "_") + "s[" + str(row_idx+1) + "] = {\n")
		...
		index = 1
		for (item_name, item) in sheet.items():
			outfp.write("Type[" + str(index) + "] = " \
				+ format_str(item_name).replace(" ", "_")  + "s\n")
			index = index + 1
			

		outfp.write("\n\n")
		outfp.write(SCRIPT_END)
		outfp.close()
